...
class EntityAttentionLSTM(object):
    def __init__(self,
                 sequence_length,
                 num_classes,
                 vocab_size,
        ...
        self.input_sememe = tf.placeholder(tf.int32, shape=[None, sememe_length], name='input_sememe')
        ...
        with tf.variable_scope('sememe_embedding'):
            self.sememe_chars = tf.nn.embedding_lookup(self.W_text, self.input_sememe)
        ...
        with tf.variable_scope('dropout_embeddings'):
        ...
        with tf.variable_scope('attention'):
            self.attn, self.alphas, = attention(
                self.rnn_outputs,
                self.input_e1, self.input_e2,
                self.p1, self.p2,
                self.sememe_chars, attention_size=attention_size
            )
